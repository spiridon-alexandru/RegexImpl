...
bool NFA::simulate(char *s)
{
    int ch_index = 0;
    state current = initial;
    NFA const& nfa = *this;

    set<state> current_states = set<state>();
    current_states.insert(initial);

    set<state> next_states = set<state>();

    set<state>::iterator currentIterator;
    set<state>::iterator closureIterator;
    while (ch_index < strlen(s))
    {
        set<state> closure = build_eps_closure(nfa, current_states);

        for (closureIterator=closure.begin(); closureIterator != closure.end(); closureIterator++)
        {
            state now = (*closureIterator);
            for (state j = 0; j < trans_table[now].size(); j++)
            {
                if (trans_table[now][j] == s[ch_index])
                {
        ...
        for (currentIterator=next_states.begin(); currentIterator != next_states.end(); currentIterator++)
        {
            current_states.insert((*currentIterator));
        }
        next_states.clear();
    ...
    set<state> endClosure = build_eps_closure(nfa, current_states);
    for (currentIterator=endClosure.begin(); currentIterator != endClosure.end(); currentIterator++)
    {
         state current = (*currentIterator);
         if (current == final)
            return true;
    }
