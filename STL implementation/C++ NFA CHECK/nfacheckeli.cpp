/**
 * http://eli.thegreenplace.net/files/docs/forays/col4.html
 */
...
vector<char> expandBracket(vector<char> toExpand)
{
    ...
    else
    {
        ...
        {
            if (i + 1 < toExpand.size())
            {
                if (toExpand[i+1] != '-')
                {
                    expanded.push_back(toExpand[i]);
                    expanded.push_back('|');
                }
                else
                {
                    if (i + 2 < toExpand.size())
                    {
                        for (int j = toExpand[i]; j < toExpand[i+2]; j++)
                        {
                            char c = j;
...
char* expand(char* regex)
{
    vector<char> newRegex;
    ...
    for (int i = 0; i < strlen(regex); i++)
    {
        ...
        {
            int j = i + 1;
            vector<char> toExpand;
            if (j < strlen(regex) && regex[j] == ']')
            {
                toExpand.push_back(']');
                j++;
            }
            
            while(j < strlen(regex) && regex[j] != ']')
            {
                toExpand.push_back(regex[j]);
                j++;
            }
            i = j;

            vector<char> expanded = expandBracket(toExpand);

            for (j = 0; j < expanded.size(); j++)
            {
                newRegex.push_back(expanded[j]);
            }
        }
        else
        {
    ...
    char* newRegexCharArray = new char[newRegex.size() + 1];
    for (int i = 0; i < newRegex.size(); i++)
    {
        newRegexCharArray[i] = (char)newRegex[i];
    }
    newRegexCharArray[newRegex.size()] = '\0';

    return newRegexCharArray;
}
...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        cerr << "Usage: " << argv[0] << " <regex> <string> <string> ..." << endl;
        exit(1);
    }

    char *newRegex = expand(argv[1]);

    for (int i = 0; i < strlen(newRegex); i++)
    {
        cout << newRegex[i];
    }
    cout << "\n";
    
    my_scanner().init(newRegex);
 ...
 //   nfa.show();
    ...
    {
        if (nfa.simulate(argv[i]))
        {
            cout << "The string " << argv[i] << " matches!\n";
        }
        else
        {
            cout << "The string " << argv[i] << " doesn't match!\n";
        }
    }
